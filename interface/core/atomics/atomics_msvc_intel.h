#ifndef HD_INC_CORE_ATOMICS_ATOMICS_MSVC_INTEL_H
#define HD_INC_CORE_ATOMICS_ATOMICS_MSVC_INTEL_H
...
/** Limits the compiler optimizations that can reorder memory accesses across the point of the call. */
    ...
    private:
        ...
        template<typename type_t>
        requires(is_integral_v<type_t>)
        static void store_no_barrier(type_t &destination, type_t value) noexcept
        {
            check(hud::memory::is_pointer_aligned(&destination, sizeof(type_t)));
            if constexpr (sizeof(type_t) == 1)
            {
        ...
        template<typename type_t>
        requires(is_integral_v<type_t>)
        [[nodiscard]] static type_t load_no_barrier(const type_t &source) noexcept
        {
