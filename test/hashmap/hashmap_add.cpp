        ...
        const auto test = []()
        {
            ...
            const auto first_element_result = std::tuple {
                *it_1st,                                  // 0
                map.count(),                              // 1
                map.max_count(),                          // 2
                it_1st->key().copy_constructor_count(),   // 3
                it_1st->value().copy_constructor_count(), // 4
                map.allocator().allocation_count(),       // 5
                map.allocator().free_count(),             // 6
            };
            ...
            const auto same_element_result = std::tuple {
                *it_same,                                  // 0
                map.count(),                               // 1
                map.max_count(),                           // 2
                it_same->key().copy_constructor_count(),   // 3
                it_same->value().copy_constructor_count(), // 4
                map.allocator().allocation_count(),        // 5
                map.allocator().free_count(),              // 6
            };
            ...
            const auto second_element_result = std::tuple {
                *it_2nd,                                  // 0
                map.count(),                              // 1
                map.max_count(),                          // 2
                it_2nd->key().copy_constructor_count(),   // 3
                it_2nd->value().copy_constructor_count(), // 4
                map.allocator().allocation_count(),       // 5
                map.allocator().free_count(),             // 6
            };
        ...
        const auto test = []()
        {
            ...
            const auto first_element_result = std::tuple {
                *it_1st,                            // 0
                map.count(),                        // 1
                map.max_count(),                    // 2
                map.allocator().allocation_count(), // 3
                map.allocator().free_count(),       // 4
            };
...
GTEST_TEST(hashmap, add_cby_move_construct_non_bitwise_copy_constructible_type)
{
    using key_type = hud_test::non_bitwise_copy_constructible_type;
    using value_type = hud_test::non_bitwise_copy_constructible_type;
    using hashmap_type = hud::hashmap<key_type, value_type, hud::hashmap_default_hasher<key_type>, hud::hashmap_default_key_equal<key_type>, hud_test::allocator_watcher<1>>;
...
GTEST_TEST(hashmap, add_inplace_by_copy_construct_non_bitwise_copy_constructible_same_type)
{
    using key_type = hud_test::non_bitwise_copy_constructible_type;
    using value_type = hud_test::non_bitwise_copy_constructible_type;
    using hashmap_type = hud::hashmap<key_type, value_type, hud::hashmap_default_hasher<key_type>, hud::hashmap_default_key_equal<key_type>, hud_test::allocator_watcher<1>>;
...
GTEST_TEST(hashmap, add_inplace_by_copy_construct_non_bitwise_copy_constructible_different_type)
{
    using key_type = hud_test::non_bitwise_copy_constructible_type2;
    using other_key_type = hud_test::non_bitwise_copy_constructible_type;
    using value_type = hud_test::non_bitwise_copy_constructible_type2;
    ...
    // With reallocation
    {
        ...
        {
            hashmap_type map;
            const auto it_1st = map.add(other_key_type(1), other_value_type(11));
            ...
            const auto it_same = map.add(other_key_type(1), other_value_type(00));
            // Add same element
            const auto same_element_result = std::tuple {
                *it_same,                                  // 0
                map.count(),                               // 1
            ...
            const auto it_2nd = map.add(other_key_type(2), other_value_type(22));
            // Add 2nd element
            const auto second_element_result = std::tuple {
                *it_2nd,                                  // 0
                map.count(),                              // 1
    ...
    // Without reallocation
    {
        ...
        // Constant
        {
            ...
            hud_assert_eq(std::get<1>(same_element_result), 1u);
            hud_assert_eq(std::get<2>(same_element_result), 3u);
            hud_assert_eq(std::get<3>(same_element_result), 2);
            hud_assert_eq(std::get<4>(same_element_result), 1u);
            hud_assert_eq(std::get<5>(same_element_result), 2u);
            hud_assert_eq(std::get<6>(same_element_result), 0u);
...
GTEST_TEST(hashmap, add_inplace_by_move_construct_non_bitwise_move_construtible_same_type)
{
    using key_type = hud_test::non_bitwise_move_constructible_type;
    using value_type = hud_test::non_bitwise_move_constructible_type;
    using hashmap_type = hud::hashmap<key_type, value_type, hud::hashmap_default_hasher<key_type>, hud::hashmap_default_key_equal<key_type>, hud_test::allocator_watcher<1>>;
    ...
    // With reallocation
    {
        ...
        {
            hashmap_type map;
            const auto it_1st = map.add(key_type {1}, value_type {11});
            ...
            const auto it_same = map.add(key_type {1}, value_type {00});
            // Add same element
            const auto same_element_result = std::tuple {
            ...
            const auto it_2nd = map.add(key_type {2}, value_type {22});
            // Add 2nd element
            const auto second_element_result = std::tuple {
...
GTEST_TEST(hashmap, add_inplace_by_move_construct_non_bitwise_move_construtible_different_type)
{
