...
GTEST_TEST(hash, crc32_hash_unaligned)
{
    ...
    GTEST_ASSERT_EQ(result2, 0x4C2750BDu);
}

GTEST_TEST(hash, crc32_result_same_as_intel_implementation)
{
    u32 runningcrc = ~0u;
    GTEST_ASSERT_EQ(hud::hash_algorithm::crc32::hash((const u8 *)(txt), hud::cstring::length(txt), 0), ~crc32_sb8_64_bit(&runningcrc, (const u8 *)(txt), static_cast<uint32_t>(hud::cstring::length(txt)), 0, MODE_CONT));

    GTEST_ASSERT_EQ(hud::hash_algorithm::crc32::hash((const u8 *)(txt_unaligned + 1), hud::cstring::length(txt_unaligned + 1), 0), ~crc32_sb8_64_bit(&runningcrc, (const u8 *)(txt_unaligned + 1), static_cast<uint32_t>(hud::cstring::length(txt_unaligned + 1)), 0, MODE_CONT));
}
