...
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_same_type_same_allocator_no_extra)
{
...
// void assign(array &&other, const usize min_slack = 0)
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_same_type_same_allocator_with_extra)
{
    ...
    // Test without extra
    {
        const auto test = [](std::initializer_list<type> elements_in_assigned, std::initializer_list<type> elements_to_assign)
        {
            ...
            assigned.assign(hud::move(to_assign), extra_assign);
...
// void assign(array &&other, const usize min_slack = 0)
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_same_type_different_allocator_no_extra)
{
...
// void assign(array &&other, const usize min_slack = 0)
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_same_type_different_allocator_with_extra)
{
...
// void assign(array &&other, const usize min_slack = 0)
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_different_type_same_allocator_no_extra)
{
    using type1 = i32;
    using type2 = u32;
    ...
    auto test_assign = [](std::initializer_list<type1> elements_in_assigned, usize extra, std::initializer_list<type2> elements_to_assign, usize extra_to_assign) noexcept
    {
        ...
        assigned.assign(std::move(to_assign));
...
// void assign(array &&other, const usize min_slack = 0)
GTEST_TEST(array, move_assign_array_of_bitwise_move_assignable_different_type_same_allocator_with_extra)
{
    using type1 = i32;
    using type2 = u32;
    // Ensure we test with different types
    ...
    auto test_assign = [](std::initializer_list<type1> elements_in_assigned, usize extra, std::initializer_list<type2> elements_to_assign, usize extra_to_assign) noexcept
    {
        ...
        /****** void assign(array &&other, const usize min_slack = 0) */
        assigned.assign(std::move(to_assign), extra_assign);

        // We should have the same count as element to assign
        hud_assert_eq(assigned.count(), static_cast<usize>(elements_to_assign.size()));
        // We should have at least the min_slack as slack
        hud_assert_ge(assigned.slack(), hud::max(extra_to_assign, extra_assign));
        hud_assert_ge(assigned.max_count(), elements_to_assign.size() + hud::max(extra_to_assign, extra_assign));
        ...
        bool is_reallocation_should_be_done = false;
        bool is_allocation_stolen = extra_to_assign >= extra_assign;
        if (!is_allocation_stolen)
        {
            is_reallocation_should_be_done = elements_to_assign.size() + hud::max(extra_to_assign, extra_assign) > elements_in_assigned.size() + extra;
        }
        if (is_reallocation_should_be_done)
        {
        ...
        if (elements_in_assigned.size() + extra > 0 && (is_reallocation_should_be_done || is_allocation_stolen))
        {
            hud_assert_eq(assigned.allocator().free_count(), previous_free_count + 1);
        }
        else
...
// // array &operator=(array<u_type_t, u_allocator_t> &&other)
// GTEST_TEST(array, move_assign_array_of_non_bitwise_move_assignable_different_type_same_allocator)
// {
...
// // array &operator=(array<u_type_t, u_allocator_t> &&other)
// GTEST_TEST(array, move_assign_array_of_non_bitwise_move_assignable_different_type_different_allocator)
// {
...
// GTEST_TEST(array, move_assign_array_same_allocator_call_destructor_of_elements)
// {
...
// // array &operator=(array<u_type_t, u_allocator_t> &&other)
// GTEST_TEST(array, move_assign_array_different_allocator_call_destructor_of_elements)
// {
