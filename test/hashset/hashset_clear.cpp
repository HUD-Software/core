#include <core/containers/hashset.h>
#include "../misc/allocator_watcher.h"

GTEST_TEST(hashset, clear_trivially_destructible_empty_map)
{
    const auto test = []()
    {
        using key_type = i32;
        hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
        set.clear();

        i32 count {0};
        for (const auto &_ : set)
        {
            count++;
        }
        return std::tuple {
            set.count() == 0,                        // 0
            set.max_count() == 0,                    // 1
            set.allocator().allocation_count() == 0, // 2
            set.allocator().free_count() == 0,       // 3
            count == 0                               // 4
        };
    };

    // Non constant
    {
        const auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
    }

    // Constant
    {
        constexpr auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
    }
}

GTEST_TEST(hashset, clear_trivially_destructible_non_empty_map)
{

    const auto test = []()
    {
        using key_type = i32;
        hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
        set.reserve(256);
        for (u32 index = 0; index < 256; index++)
        {
            set.add(index);
        }

        set.clear();
        i32 count {0};
        for (const auto &_ : set)
        {
            count++;
        }
        return std::tuple {
            set.count() == 0,                                                             // 0
            set.max_count() > set.count(),                                                // 1
            set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
            set.allocator().free_count() == 0,                                            // 3
            count == 0                                                                    // 4
        };
    };

    // Non constant
    {
        const auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
    }

    // Constant
    {
        constexpr auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
    }
}

GTEST_TEST(hashset, clear_non_trivially_destructible_empty_map)
{

    // No memory allocated
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            set.clear();

            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }
            return std::tuple {
                set.count() == 0,                        // 0
                set.max_count() == 0,                    // 1
                set.allocator().allocation_count() == 0, // 2
                set.allocator().free_count() == 0,       // 3
                count == 0                               // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }

    // Memory reserved
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 256;
            set.reserve(COUNT);
            set.clear();

            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }
            return std::tuple {
                set.count() == 0,                                                             // 0
                set.max_count() > COUNT,                                                      // 1
                set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
                set.allocator().free_count() == 0,                                            // 3
                count == 0                                                                    // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }
}

GTEST_TEST(hashset, clear_non_trivially_destructible_non_empty_map)
{

    const auto test = []()
    {
        using key_type = hud_test::non_bitwise_type;
        hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;

        constexpr usize COUNT = 256;
        set.reserve(COUNT);
        i32 key_destructor_called_count[COUNT];

        // Add all elements
        for (i32 index = 0; index < COUNT; index++)
        {
            set.add(
                key_type {index, key_destructor_called_count + index}
            );
        }
        hud::memory::set_memory_safe(key_destructor_called_count, 0);

        // Clear the hashset
        set.clear();

        // Ensure we have no element left in the set
        i32 count {0};
        for (const auto &_ : set)
        {
            count++;
        }

        // Ensure all destructor are called once
        bool all_destructor_called_after_clear = true;
        for (i32 index = 0; index < COUNT; index++)
        {
            if (key_destructor_called_count[index] != 1)
            {
                all_destructor_called_after_clear = false;
                break;
            }
        }

        return std::tuple {
            set.count() == 0,                                                             // 0
            set.max_count() > set.count(),                                                // 1
            set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
            set.allocator().free_count() == 0,                                            // 3
            count == 0,                                                                   // 4
            all_destructor_called_after_clear                                             // 5
        };
    };

    // Non constant
    {
        const auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
        hud_assert_true(std::get<5>(result));
    }

    // Constant
    {
        constexpr auto result = test();
        hud_assert_true(std::get<0>(result));
        hud_assert_true(std::get<1>(result));
        hud_assert_true(std::get<2>(result));
        hud_assert_true(std::get<3>(result));
        hud_assert_true(std::get<4>(result));
        hud_assert_true(std::get<5>(result));
    }
}

GTEST_TEST(hashset, clear_then_add_trivially_destructible_empty_map)
{

    // No reserve
    {
        const auto test = []()
        {
            using key_type = i32;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 256;
            set.clear();

            for (u32 index = 0; index < COUNT; index++)
            {
                set.add(index);
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT,                   // 0
                set.max_count() > COUNT,                // 1
                set.allocator().allocation_count() > 0, // 2
                set.allocator().free_count() > 0,       // 3
                count == COUNT                          // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }

    // With reserve
    {
        const auto test = []()
        {
            using key_type = i32;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 256;
            set.reserve(COUNT);
            set.clear();

            for (u32 index = 0; index < COUNT; index++)
            {
                set.add(index);
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT,                   // 0
                set.max_count() > COUNT,                // 1
                set.allocator().allocation_count() > 0, // 2
                set.allocator().free_count() == 0,      // 3
                count == COUNT                          // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }
}

GTEST_TEST(hashset, clear_then_add_trivially_destructible_non_empty_map)
{
    // No reserve
    {
        const auto test = []()
        {
            using key_type = i32;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            // Add elements then clear the set
            constexpr usize COUNT = 128;
            for (u32 index = 0; index < COUNT; index++)
            {
                set.add(index);
            }
            set.clear();

            // Add elements again
            for (u32 index = 0; index < COUNT * 2; index++)
            {
                set.add(index);
            }

            // Check that we can iterate over all elements
            i32 element_count {0};
            for (const auto &_ : set)
            {
                element_count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,               // 0
                set.max_count() > set.count(),          // 1
                set.allocator().allocation_count() > 0, // 2
                set.allocator().free_count() > 0,       // 3
                element_count == COUNT * 2              // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }

    // Reserve
    {
        const auto test = []()
        {
            using key_type = i32;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            // Add elements then clear the set
            constexpr usize COUNT = 128;
            set.reserve(COUNT * 2);
            for (u32 index = 0; index < COUNT; index++)
            {
                set.add(index);
            }
            set.clear();

            // Add elements again
            for (u32 index = 0; index < COUNT * 2; index++)
            {
                set.add(index);
            }

            // Check that we can iterate over all elements
            i32 element_count {0};
            for (const auto &_ : set)
            {
                element_count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,                                                     // 0
                set.max_count() > set.count(),                                                // 1
                set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
                set.allocator().free_count() == 0,                                            // 3
                element_count == COUNT * 2                                                    // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }
}

GTEST_TEST(hashset, clear_then_add_non_trivially_destructible_empty_map)
{

    // No reserve
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 128;
            set.clear();

            // Add all elements
            for (i32 index = 0; index < COUNT * 2; index++)
            {
                set.add(key_type {index});
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,               // 0
                set.max_count() > COUNT,                // 1
                set.allocator().allocation_count() > 0, // 2
                set.allocator().free_count() > 0,       // 3
                count == COUNT * 2                      // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }

    // With reserve
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 128;
            set.reserve(COUNT * 2);
            set.clear();

            // Add all elements
            for (i32 index = 0; index < COUNT * 2; index++)
            {
                set.add(key_type {index});
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,                                                     // 0
                set.max_count() > COUNT,                                                      // 1
                set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
                set.allocator().free_count() == 0,                                            // 3
                count == COUNT * 2                                                            // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }
}

GTEST_TEST(hashset, clear_then_add_non_trivially_destructible_non_empty_map)
{

    // No reserve
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 128;

            // Add all elements
            for (i32 index = 0; index < COUNT; index++)
            {
                set.add(key_type {index});
            }

            set.clear();

            // Add all elements
            for (i32 index = 0; index < COUNT * 2; index++)
            {
                set.add(key_type {index});
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,               // 0
                set.max_count() > COUNT,                // 1
                set.allocator().allocation_count() > 0, // 2
                set.allocator().free_count() > 0,       // 3
                count == COUNT * 2                      // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }

    // With reserve
    {
        const auto test = []()
        {
            using key_type = hud_test::non_bitwise_type;
            hud::hashset<key_type, hud::hash_64<key_type>, hud::equal<key_type>, hud_test::allocator_watcher<1>> set;
            constexpr usize COUNT = 128;
            set.reserve(COUNT * 2);

            // Add all elements
            for (i32 index = 0; index < COUNT; index++)
            {
                set.add(key_type {index});
            }

            set.clear();

            // Add all elements
            for (i32 index = 0; index < COUNT * 2; index++)
            {
                set.add(key_type {index});
            }

            // Check that we can iterate over all elements
            i32 count {0};
            for (const auto &_ : set)
            {
                count++;
            }

            return std::tuple {
                set.count() == COUNT * 2,                                                     // 0
                set.max_count() > COUNT,                                                      // 1
                set.allocator().allocation_count() == (hud::is_constant_evaluated() ? 2 : 1), // 2
                set.allocator().free_count() == 0,                                            // 3
                count == COUNT * 2                                                            // 4
            };
        };

        // Non constant
        {
            const auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }

        // Constant
        {
            constexpr auto result = test();
            hud_assert_true(std::get<0>(result));
            hud_assert_true(std::get<1>(result));
            hud_assert_true(std::get<2>(result));
            hud_assert_true(std::get<3>(result));
            hud_assert_true(std::get<4>(result));
        }
    }
}